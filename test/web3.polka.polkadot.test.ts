...
describe('test some RPC methods at web3.polka.polkadot', () => {
  let web3: Web3;
  ...
  it('should call chain.getBlock method', async () => {
    ...
    const response = await web3.polka.polkadot.rpc.chain.getBlock(/*hash*/);
  ...
  it('should chain.getBlock() equals chain.getBlock(chain.getBlockHash(latestBlocHash))', async () => {
    const latestBlock = await web3.polka.polkadot.rpc.chain.getBlock();
    const latestBlocHash = await web3.polka.polkadot.rpc.chain.getBlockHash(latestBlock.block.header.number.toString());
    const blockByLatestBlocHash = await web3.polka.polkadot.rpc.chain.getBlock(latestBlocHash);
  ...
  it('should get the rpc methods', async () => {
    const response = await web3.polka.polkadot.rpc.rpc.methods();
