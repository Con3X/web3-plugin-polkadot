...
describe('test some RPC methods at web3.polka.kusama', () => {
  ...
  it('should call chain.getBlock method', async () => {
    ...
    const response = await web3.polka.kusama.rpc.chain.getBlock(/*hash*/);
  ...
  it('should chain.getBlock() equals chain.getBlock(chain.getBlockHash(latestBlocHash))', async () => {
    const latestBlock = await web3.polka.kusama.rpc.chain.getBlock();
    const latestBlocHash = await web3.polka.kusama.rpc.chain.getBlockHash(latestBlock.block.header.number.toString());
    const blockByLatestBlocHash = await web3.polka.kusama.rpc.chain.getBlock(latestBlocHash);
  ...
  it('should get the rpc methods', async () => {
    const response = await web3.polka.kusama.rpc.rpc.methods();
